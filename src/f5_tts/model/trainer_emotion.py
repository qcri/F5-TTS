...
class TrainerConditioned:
    ...
    def _save_current_model_sample(
        self,
        vocoder,
        ref_audio,
        ref_audio_len,
        ...
        torchaudio.save(ref_wav_path, ref_audio.cpu(), target_sample_rate)
        cfg_strength2_list = [None, 10, 20]
        for emotion in [
            [[emotion_inputs[0][0], "Happy"]],
            [[emotion_inputs[0][0], "Sad"]],
            [[emotion_inputs[0][0], "Angry"]],
            [[emotion_inputs[0][0], "Neutral"]],
            [[emotion_inputs[0][0], "Surprise"]],
        ]:
            for cfg_strength2 in cfg_strength2_list:
                ...
                if masking_type == "original":
                    ...
                    cond = mel_spec[0, : int((first_phrase_length[0] * ref_audio_len / len(text_inputs[0])))]
                    generated, _ = self.accelerator.unwrap_model(self.model).sample(
                    ...
                    generated = generated.to(torch.float32)
                    gen_audio = vocoder.decode(generated[:, :, :].permute(0, 2, 1).to(self.accelerator.device))

                torchaudio.save(gen_wav_path_new, gen_audio.cpu(), target_sample_rate)
    ...
    def _validation(
        ...
        from f5_tts.model.metrics import (
            Calculate_MCD_from_ndarray,
            WhisperModelAdapter,
            get_error,
            link_words,
        ...
        if compute_wer:
            asr_tool = WhisperModelAdapter(faster_whisper_path, text_language, "float16", "cpu")
        ...
        for batch in tqdm(dataloader):
            ...
            if masking_type == "original":
                ...
                gt_text = text_inputs[0][first_phrase_length[0] :]
                generated = generated.to(torch.float32)[
                    :, int((first_phrase_length[0] * ref_audio_len / len(text_inputs[0]))) :, :
                ]

            generated, mel_spec = self.remove_leading_value(generated, mel_spec)

            gen_audio = vocoder.decode(generated.permute(0, 2, 1).to(self.accelerator.device))
            ref_audio = vocoder.decode(mel_spec.permute(0, 2, 1).to(self.accelerator.device))
            ...
            if val_step % 1 == 0:
                ...
                self._save_current_model_sample(
                    vocoder,
                    ref_audio,
                    ref_audio_len,
                    text_inputs,
            ...
            util_ref_audio = ref_audio[:, -gen_audio.shape[1] :]
            if compute_mcd:
                mcd = mcd_toolbox.calculate_mcd(
                    util_ref_audio.squeeze(0).to("cpu").numpy(),
                    gen_audio.squeeze(0).to("cpu").numpy(),
                    audios_sample_rate=target_sample_rate,
                )
    ...
    def train(
        ...
        **kwargs,
        ...
        train_dataloader = self._return_dataloader(
            train_dataset, self.batch_size, num_workers, collate_fn_emotion, generator, resumable_with_seed
        )
        val_dataloader = self._return_dataloader(
            val_dataset, 1, num_workers, collate_fn_emotion, generator, resumable_with_seed
        )
        ...
        # Initialize emotion weights for text_mirror conditioning
        if kwargs["emotion_conditioning"]["emotion_condition_type"] == "text_mirror":
            ...
            if kwargs["emotion_conditioning"]["load_emotion_weights"]:
                self.model.transformer.input_embed._initialize_proj_emotion_weights(
                    init_type, weight_reduction_scale
                )
            ...
            if kwargs["perform_validation"] and kwargs["pre_valid"]:
                with torch.no_grad():
                    self._validation(
                        ...
                        asr_device,
                        val_samples_path,
                        kwargs["compute_mcd_valid"],
                        kwargs["compute_wer_valid"],
                        masking_type,
                        kwargs["noise_2ndhalf"],
                    )
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    if kwargs["emotion_conditioning_kwargs"]["contrastive_loss"] or kwargs["change_emotion_forward"]:
                        ...
                        def pad_to_match(tensor1, tensor2):
                            ...
                            if x1 < x2:
                                return F.pad(tensor1, (0, 0, 0, x2 - x1)), tensor2
                            elif x2 < x1:
                                return tensor1, F.pad(tensor2, (0, 0, 0, x1 - x2))
                            else:
                                return tensor1, tensor2

                        mel_spec, mel_spec2 = pad_to_match(mel_spec, mel_spec2)
                        mel_lengths = mel_lengths2 = (
                            mel_lengths if mel_lengths.item() > mel_lengths2.item() else mel_lengths2
                        )
                        ...
                        loss, cond, pred, melspec_predicted, noised_base = self.model(
                            ...
                            text=text_inputs,
                            first_phrase_length=first_phrase_length,
                            emotion=emotion_pair,
                            lens=mel_lengths,
                            noise_scheduler=self.noise_scheduler,
                            masking_type=masking_type,
                            noise_2ndhalf=kwargs["noise_2ndhalf"],
                        )
                    else:
                        loss, cond, pred, melspec_predicted, noised_base = self.model(
                            mel_spec,
                    ...
                    if kwargs["emotion_conditioning"]["emotion_condition_type"] == "text_mirror":
                        weights_grad = self.model.transformer.input_embed.proj_emotion.weight.grad[
                            :, self.model.transformer.input_embed.proj.weight.shape[1] :
                        ]
                        avg_abs_grad_emo = weights_grad.abs().mean()
                        self.accelerator.log(
                            {"Mean Abs Grad emotion weights": avg_abs_grad_emo.item()}, step=global_step
                        )
                        weights_grad = self.model.transformer.input_embed.proj_emotion.weight.grad[
                            :, : self.model.transformer.input_embed.proj.weight.shape[1]
                        ]
                        ...
                        self.accelerator.log(
                            {"Mean Abs Grad non-emotion weights": avg_abs_grad.item()}, step=global_step
                        )
